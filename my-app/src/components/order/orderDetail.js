...
export default function OrderDetail() {
    ...
    const { payment, orderStatus, color, statusColor } = useMemo(() => {
        ...
        if (orderDetail?.booking?.paymentStatus === 'Denied') {
            payment = 'Denied';
            color = 'redColor';
        } else if (orderDetail?.booking?.paymentStatus === 'Verified') {
            payment = 'Paid';
            color = 'greenColor';
        }

        if (orderDetail?.booking?.status === 'Cancelled') {
            orderStatus = 'Cancelled';
            statusColor = 'redColor';
        } else if (orderDetail?.booking?.status === 'Confirmed') {
    ...
    const isPaid = orderDetail?.booking?.paymentStatus === 'verified' ? true : false;
                ...
                <Fragment>
                    ...
                    <div ref={pdfRef} className="row d-flex justify-content-between" >
                        <div className="col-12 col-lg-8 mt-5 order-details">
                            ...
                            <p><b>Amount:</b> <span style={{fontStyle:"italic", fontWeight:"bold", color:"red"}}>Rs.{orderDetail?.booking?.totalAmount}</span></p>
                            ...
                            <div className="cart-item" style={{ fontFamily: 'Arial, sans-serif' }}>
                                {orderDetail.booking?.tours?.map((item, index) => (
                                    <div
                                        key={index}
                                    ...
                                    >
                                        ...
                                        <div className="col-2 d-flex justify-content-center pe-0">
                                            <img
                                                src={item.tourId.images[0].image}
                                        ...
                                        <div className="col-2 pe-1">
                                            <div className="d-flex align-items-center h-100">
                                                ...
                                                >
                                                   <b> Rs.{item.tourId.price.toLocaleString()}</b>
                                                </span>
                                            </div>
                                        </div>
                                        ...
                                        <div className="col-3">
                                            <div className="d-flex align-items-center justify-content-end h-100">
                                                ...
                                                >
                                                    {item.spotsBooked} Spot{item.spotsBooked !== 1 ? 's' : ''}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
