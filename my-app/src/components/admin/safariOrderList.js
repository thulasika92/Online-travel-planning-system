...
import { deleteOrder, adminBookings as adminOrderActions } from "../../actions/safariOrderAction"
import { clearError, clearOrderDeleted } from "../../slices/safariOrderSlice";
import { toast } from "react-toastify";
import Sidebar from "./Sidebar";
import { Button } from "react-bootstrap";

export default function SafariOrderList() {
    const { adminSafariOrder = [], loading = true, error, isOrderDeleted } = useSelector(state => state.safariOrderState);
    ...
    const setOrders = () => {
        ...
        adminSafariOrder.forEach(order => {
            ...
            data.rows.push({
                ...
                actions: (
                    <Fragment>
                        <Link to={`/admin/safariOrder/${order._id}`} className="btn btn-primary"><i className="fa fa-pencil"></i></Link>
    ...
    return (
        <div className="row">
            ...
            <div className="col-12 col-md-10">
                <h1 className="my-4">Safari Bookings List</h1>
                <Fragment>
                    {loading ? <Loader /> :
                        <MDBDataTable
                            data={setOrders()}
